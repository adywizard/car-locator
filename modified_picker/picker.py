    ...
    MDRelativeLayout:
        ...
        TimeInputLabel:
            ...
            text_color:
                ...
                app.theme_cls.secondary_text_color
...
class TimeInputTextField(MDTextField):
    ...
    hour_regx = "^[0-9]$|^0[0-9]$|^[0-2][0-2]$"
    minute_regx = "^[0-9]$|^0[0-9]$|^[0-9][0-9]$"
    c = None

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        Clock.schedule_once(self._set_padding)
    ...
    def _set_padding(self, *_):
        ...
        if len(self.text) == 1 and self.cursor_col == 0 and int(self.text) > 5 and self.num_type == 'minute':
            self.cursor = [0, 0]

        if len(self.text) == 1 and self.cursor_col == 0 and int(self.text) > 2 and self.num_type == 'hour':
            self.cursor = [0, 0]

        self._refresh_text(self.text)
        max_size = max(self._lines_rects, key=lambda r: r.size[0]).size
        dx = (self.width - max_size[0]) / 2.0
        dy = (self.height - max_size[1]) / 2.0111
        self.padding = [dx, dy, dx, dy]

        if len(self.text) >= 2:
            self.cursor = [0, 0]
            self.cursor = [2, 0]
            if int(self.text[0]) > 5 and self.num_type == 'minute':
                self.text = '00'
            elif int(self.text) > 12 and self.num_type == 'hour':
                self.text = '00'
        self.c.cancel()
        self.c = None
    ...
    def on_text(self, *args):
        ...
        """
        if not self.c:
            self.c = Clock.schedule_once(self._set_padding, 0)
